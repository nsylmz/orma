...
public class VegaDokumanReader {
	...
	public static void main(String[] args) throws IOException {
		readDokuman("D:\\Enes\\Springsource\\test data\\input\\BUFE02.xls", "BUFE02", false);
	}
	
	public static VegaDokuman readDokuman(String filePath, String sheetName, boolean processed) throws IOException {
		...
		while (rowIterator.hasNext()) {
			...
			if(!dokumanBasladi) {
				...
				if (faturaBitti) { // Fatura Bitti
					...
					fatura.setEvrakNo(BigDecimal.valueOf(row.getCell(2).getNumericCellValue()));
					if (!processed) {
		...
		System.out.println(stream.toXML(dokuman));
		return dokuman;
	}
